...
seqan::ArgumentParser::ParseResult parseCommandLine(AppOptions & options, int argc, char const ** argv) {
	...
	setVersion(parser, "0.3-Exact-Matching");
	setDate(parser, "October 2013");

	// Define usage line and long description.
	addUsageLine(parser, "[\\fIOPTIONS\\fP] SEQUENCE_FILE OUTPUT_DIRECTORY");
	addDescription(
			parser,
			"RAIDER parses the given sequence file to identify de novo elementary repeats. Minimum repeat size and other options can be configured as described below.");
	...
	addOption(
			parser,
			seqan::ArgParseOption("m", "min", "Minimum repeat length. Defaults to 24.",
	...
	addListItem(parser, "\\fBraider\\fP \\fB-v\\fP \\fIchr23.fasta\\fP chr23_out",
			"Call with chromosome 23 and verbose output.");

	// Only extract options if the program will continue after parseCommandLine()
	if (seqan::parse(parser, argc, argv) != seqan::ArgumentParser::PARSE_OK)
		return seqan::ArgumentParser::PARSE_ERROR;
	// Extract option values / set defaults
...
int main(int argc, char const ** argv) {
	...
	getElementaryFamilies(sequence, options.count, options.min, families);
