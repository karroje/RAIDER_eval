...
uint minLength(const vector<seqan::CharString> &seeds) {
  uint min = std::numeric_limits<int>::min();
  for (int i = 0; i < seeds.size(); i++) {
    uint len = seqan::length(seeds[i]);
    if (len < min) {
      min = len;
    }
  }
...
seqan::ArgumentParser::ParseResult parseCommandLine(AppOptions & options, int argc, char const ** argv) {
  ...
  addOption(
            ...
            seqan::ArgParseOption("s", "seed", "Spaced seed/mask to use. Defaults to 111110011111110001111111000000000000011111.",
                                  seqan::ArgParseOption::STRING, "STRING", true));
  ...
  if (isSet(parser, "seed")){
    for (int i = 0 ; i < seqan::getOptionValueCount(parser,"seed"); i++){
      seqan::CharString seed;
      seqan::getOptionValue(seed, parser, "seed", i);
    ...
    options.seeds.push_back("111110011111110001111111000000000000011111");
  
  if (isSet(parser, "min"))
    seqan::getOptionValue(options.min, parser, "min");
  else{
    if (options.seeds.size() == 1){
      options.min = seqan::length(options.seeds[0]);
    }
    else{
      options.min = minLength(options.seeds);
    }
  }
...
void printArgs(AppOptions &options) {
  if (options.verbosity > 0) {
    ...
    if (options.seeds.size() == 1){
      cout << "SPACED_SEED     \t" << options.seeds[0] <<endl;
    }
    else{
      cout << "SPACED_SEED     \t";
      for (seqan::CharString seed: options.seeds){
        cout << seed << " ";
      }
      cout << endl;
    }
		cout << "SEQUENCE_FILE\t" << options.sequence_file << endl
...
int main(int argc, char const ** argv) {
  ...
  getElementaryFamilies(sequence, options.seeds, families, options.verbosity, options.age);
