...
typedef unordered_map<std::string, LmerVector*> LmerMap;
...
bool getNextSeed(seqan::Dna5String &sequence, uint &index, uint seqLength, uint L, string& seed) {
  // Mark end with 0. If that 0 is overwritten, a full Lmer was found. 
  seed[L - 1] = 0;
...
LmerVector* getAndInsert(LmerMap &lmers, string const& seed, uint index) {
...
bool isNewFamily(Family* fam, LmerVector* v, bool overlaps) {
  // Make sure that fam is a viable family of  exactly 2
  assert(!v->getFamily());
  if (!fam || fam->size() > 2) {
    return true;
  }
...
string applyMask(seqan::CharString const& mask, uint weight, string const& unmasked) {
  string masked(weight, 0);
  int maskedCount = 0;
  for (uint i = 0; i < seqan::length(mask); i++) {
    if (mask[i] == '1') {
      masked[maskedCount++] = unmasked[i];
    }
  }
  return masked;
}
...
unordered_set<string>* createFilter(seqan::Dna5String &sequence, vector<seqan::CharString> &masks, uint weight) {
  ...
  unordered_set<string> single_occurence;
  unordered_set<string>* multi_occurence = new unordered_set<string>;
  while (getNextSeed(sequence, ++index, seqLength, L, unmasked)) {
    string seed = applyMask(mask, weight, unmasked);
...
uint getWeight(seqan::CharString const& mask, uint length) {
  uint weight = 0;
...
void getElementaryFamilies(seqan::Dna5String &sequence, vector<seqan::CharString> &masks, vector<Family*> &families, AppOptions options) {
  ...
  const uint L = seqan::length(mask);
  const uint MAX_L = L;
  const uint weight = getWeight(mask, MAX_L);
  
  uint index = -1;
  string unmasked(MAX_L, 0);
  LmerMap lmers;
  Family* fam = nullptr;
  ...
  unordered_set<string>* filter_set = options.prescan ? createFilter(sequence, masks, weight) : NULL;
