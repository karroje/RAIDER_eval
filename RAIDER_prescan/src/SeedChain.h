...
void setLmers(seqan::Dna5String &sequence, const uint seqLength, const seqan::CharString &mask, const uint MIN, const uint L, LmerMap &lmers) {
	LmerToIntMap counts;
	char unmasked[L];
	uint p = 3;
	...
	for (uint i = 0; i < p; i++) {
		...
		for (auto kv : counts) {
			if (kv.second >= MIN && lmers.count(kv.first) == 0)
